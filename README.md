...
### Файл ```transaction_filter.py```

```search_transactions```Находит транзакции по заданной строке в описании
```count_operations_by_category```Подсчитывает количество транзакций по категориям
...
#### Функция ```read_financial_operations_csv```
Считывает финансовые операции из CSV файла

#### Функция ```read_financial_operations_xlsx```
...
#### Пример тестирования ```main.py``` в ```test_main.py```
```
import unittest
...
#### Пример тестирования ```transaction_filter.py``` в ```test_transaction_filter.py```
...
```
#### Пример тестирования ```file_reader.py``` в ```test_file_reader.py```
